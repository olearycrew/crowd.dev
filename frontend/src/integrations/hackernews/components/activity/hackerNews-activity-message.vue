<template>
  <div v-if="!short && !channelOnly">
    ...
    />
    <span v-if="!channelOnly && isComment">on
      <a
        :href="activity.attributes.parentUrl"
        target="_blank"
        class="truncate"
      >
        {{ computedParentTitle }}</a></span>
    <span v-if="!channelOnly && !isComment">&nbsp;mentioning</span>
    <a
  ...
  <div v-else-if="short" class="truncate">
    ...
    <span v-else>&nbsp;mentioning {{ computedChannelShort }}
    </span>
  </div>
</template>
