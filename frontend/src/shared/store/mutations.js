...
export default () => ({
  ...
  PAGINATION_CHANGED(state, payload) {
    ...
    state.views[activeView.id].pagination = pagination || {};
  },

  PAGINATION_CURRENT_PAGE_CHANGED(state, payload) {
    const { activeView, currentPage } = payload;
    const previousPagination = state.views[activeView.id].pagination || {};
  ...
  SORTER_CHANGED(state, payload) {
    ...
    state.views[activeView.id].sorter = Object.keys(sorter).length > 0
      ? sorter
      : {
        prop: 'createdAt',
        order: 'descending',
  ...
  FILTER_ATTRIBUTE_RESETED(state, payload) {
    ...
    state.views[activeView.id].filter.attributes[
      attribute.name
    ].value = state.views[activeView.id].filter.attributes[
      attribute.name
    ].defaultValue;
    state.views[activeView.id].filter.attributes[
      attribute.name
    ].operator = state.views[activeView.id].filter.attributes[
      attribute.name
    ].defaultOperator;
  },
});
